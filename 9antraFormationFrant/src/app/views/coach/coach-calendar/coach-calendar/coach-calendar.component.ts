...
export class CoachCalendarComponent {
  ...
  retrieveSessions(): void {
    ...
    this.sessionService.getSessionsByFormateurId(formateurId).subscribe(
      (sessions: Session[]) => {
        ...
        const notStartedSession = sessions.find(session => this.adjustDateByOneHour(session.startDate).getTime() > this.currentDate.getTime());
  ...
  private updateButtonStatus(): void {
    ...
    this.isButtonDisabled = currentTime <= databaseTime || currentTime
      >= databaseTime + durationInMillis;
